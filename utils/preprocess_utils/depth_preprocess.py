...
# default is the 17 joints skeleton
...
def preprocess(joints_2d, joints_z, bone_status, bone_relations, is_training=True, bone_width=6, joint_ratio=6, bg_color=0.2, num_of_joints=17):
    settings = {
        "img_size": 256,
        "crop_box_size": 256,
    ...
    if is_training:
        ...
        annots = np.concatenate([joints_2d, joints_z[:, np.newaxis], np.concatenate([[-1], bone_status])[:, np.newaxis], old_order[:, np.newaxis]], axis=1)
        _, aug_annot = augment_data_2d(img, annots, settings)

        aug_joints_2d = aug_annot[:, 0:2]
        aug_joints_z = aug_annot[:, 2]
    ...
    return aug_img, aug_joints_2d, aug_joints_z

def flip_annots(joints_2d, joints_z, bone_status, bone_relations, size=256):
    old_order = np.arange(0, joints_2d.shape[0], 1) - 1
    ...
    flipped_joints_z = flipped_annots[:, 2]
    ...
    return flipped_joints_2d, flipped_joints_z, flipped_bone_status, flipped_bone_relations
