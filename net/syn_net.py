...
class mSynNet(object):
    def __init__(self, nJoints, is_training, batch_size, img_size=256, loss_weight_heatmap=1.0, loss_weight_synmap=20.0):

        self.loss_weight_heatmap = loss_weight_heatmap
        self.loss_weight_synmap = loss_weight_synmap_
    ...
    def build_model(self, input_images):
        with tf.variable_scope("SynNet"):
            ...
            self.synmaps = tf.layers.conv2d(inputs=lin4, filters=1, kernel_size=1, strides=1, use_bias=self.is_use_bias, padding="SAME", activation=None, kernel_initializer=tf.contrib.layers.xavier_initializer(), name="synmaps")
    ...
    def build_loss_gt(self, input_heatmaps, input_synmaps, lr, lr_decay_step, lr_decay_rate):
        ...
        with tf.variable_scope("losses"):
            ...
            self.synmap_loss = tf.nn.l2_loss(input_synmaps - self.synmaps, name="synmap_loss") / self.batch_size * self.loss_weight_synmap

            self.total_loss = self.heatmap_loss + self.synmap_loss
