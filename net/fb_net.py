...
# Currently this network only train the data with limbs, shoulders, hips
class mFBNet(object):
    def __init__(self, nJoints, is_training, batch_size, img_size=256, loss_weight_heatmaps=1.0, loss_weight_fb=1.0, is_use_bn=True):
    ...
    def build_model(self, input_images):
        with tf.variable_scope("FBNet"):
            ...
            with tf.variable_scope("final_classify"):
                ...
                # fb information
                self.results = tf.layers.dense(inputs=reg, units= 3 * (self.nJoints - 1), activation=None, kernel_initializer=tf.contrib.layers.xavier_initializer(), name="fc")
    ...
    def build_loss(self, input_heatmaps, input_fb, lr, lr_decay_step, lr_decay_rate):
        ...
        with tf.variable_scope("losses"):
            ...
            self.total_loss = self.fb_loss + self.heatmaps_loss
