...
class mPoseNet(object):
    def __init__(self, nJoints, is_training, batch_size, img_size=256, loss_weight_heatmap=1.0, loss_weight_volume=1.0, is_use_bn=True):
    ...
    def build_model(self, input_images):
        with tf.variable_scope("PoseNet"):
            ...
            lin4 = mConvBnRelu(inputs=lin3, nOut=512, kernel_size=1, strides=1, is_use_bias=self.is_use_bias, is_training=self.is_training, is_use_bn=self.is_use_bn, name="lin4")
    ...
    # PoseNet with ground truth volumes
    def build_loss(self, input_heatmaps, input_volumes, lr, lr_decay_step, lr_decay_rate):
        self.global_steps = tf.train.get_or_create_global_step()
