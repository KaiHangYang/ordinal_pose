    ...
    def _main_path(self, inputs, nOut, kernel_size=3, strides=1, name='main_path'):
        if self.is_tiny:
            with tf.variable_scope(name):
                norm = tf.contrib.layers.batch_norm(inputs, 0.9, epsilon=1e-5, activation_fn=tf.nn.relu, is_training=self.is_training, updates_collections=None)
            ...
            with tf.variable_scope(name):
                with tf.variable_scope("norm_1"):
                    norm_1 = tf.contrib.layers.batch_norm(inputs, 0.9, epsilon=1e-5, activation_fn=tf.nn.relu, is_training=self.is_training, updates_collections=None)
                    conv_1 = tf.layers.conv2d(norm_1, nOut/2, 1, 1, padding="SAME", use_bias=self.is_use_bias, name="conv")
                with tf.variable_scope("norm_2"):
                    norm_2 = tf.contrib.layers.batch_norm(conv_1, 0.9, epsilon=1e-5, activation_fn=tf.nn.relu, is_training=self.is_training, updates_collections=None)
                ...
                with tf.variable_scope("norm_3"):
                    norm_3 = tf.contrib.layers.batch_norm(conv_2, 0.9, epsilon=1e-5, activation_fn=tf.nn.relu, is_training=self.is_training, updates_collections=None)
