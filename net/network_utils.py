...
class mResidualUtils(object):
    def __init__(self, is_training=True, is_tiny=False, is_use_bias=True, is_bn_offset=True, is_bn_scale=True):
        ...
        self.is_bn_scale = is_bn_scale
        self.is_bn_offset = is_bn_offset
    ...
    def _main_path(self, inputs, nOut, kernel_size=3, strides=1, name='main_path'):
        if self.is_tiny:
            with tf.variable_scope(name):
                ...
                conv = tf.layers.conv2d(norm, nOut, kernel_size, strides, padding="SAME", use_bias=self.is_use_bias, kernel_initializer=tf.initializers.truncated_normal(stddev=truncated_normal_dev), name="conv")
                return conv
        else:
            with tf.variable_scope(name):
                with tf.variable_scope("norm_1"):
                    ...
                    conv_1 = tf.layers.conv2d(norm_1, nOut/2, 1, 1, padding="SAME", use_bias=self.is_use_bias, kernel_initializer=tf.initializers.truncated_normal(stddev=truncated_normal_dev), name="conv")
                with tf.variable_scope("norm_2"):
                    ...
                    conv_2 = tf.layers.conv2d(norm_2, nOut/2, kernel_size, strides, padding="SAME", use_bias=self.is_use_bias, kernel_initializer=tf.initializers.truncated_normal(stddev=truncated_normal_dev), name="conv")
                with tf.variable_scope("norm_3"):
                    ...
                    conv_3 = tf.layers.conv2d(norm_3, nOut, 1, 1, padding="SAME", use_bias=self.is_use_bias, kernel_initializer=tf.initializers.truncated_normal(stddev=truncated_normal_dev), name="conv")
    ...
    def _skip_path(self, inputs, nOut, strides=1, name='skip_path'):
        with tf.variable_scope(name):
            if inputs.get_shape().as_list()[3] == nOut and strides == 1:
                ...
                conv = tf.layers.conv2d(inputs, nOut, 1, strides, padding="SAME", use_bias=self.is_use_bias, kernel_initializer=tf.initializers.truncated_normal(stddev=truncated_normal_dev), name="conv")
                return conv
...
def mConvBnRelu(inputs, nOut, kernel_size=1, strides=1, name="conv_bn_relu", is_training=True, is_use_bias=True):
    with tf.variable_scope(name):
        conv = tf.layers.conv2d(
                    ...
                    kernel_initializer=tf.initializers.truncated_normal(stddev=truncated_normal_dev),
                    name="conv")
