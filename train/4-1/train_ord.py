...
# ver means version
configs.parse_configs(t=1, ver=1)
...
if __name__ == "__main__":
    ...
    ordinal_model = ordinal_F.mOrdinal_F(nJoints=configs.nJoints, img_size=configs.img_size, batch_size=input_batch_size, is_training=input_is_training, loss_weight_heatmap=10.0, loss_weight_volume=1.0, loss_weight_rank=1.0, loss_weight_2d=1000.0)

    with tf.Session() as sess:

        with tf.device("/device:GPU:0"):
            ordinal_model.build_model(input_images)
            input_heatmaps = ordinal_model.build_input_heatmaps(input_centers_2d, stddev=2.0, gaussian_coefficient=True)
        ...
        while True:
            ...
            if is_valid:
                ...
                summary  = sess.run(
                        [
                         ...
                         ordinal_model.loss_rank,
                         ordinal_model.loss_2d,
            ...
            print("Iteration: {:07d} \nlearning_rate: {:07f} \nRank Loss : {:07f}\n2D Loss: {:07f}\nHeatmap Loss: {:07f}\n\n".format(global_steps, lr, loss_rank, loss_2d, loss_hm))
