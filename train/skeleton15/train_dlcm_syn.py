...
if __name__ == "__main__":
    ...
    dlcm_syn_model = dlcm_syn_net.mDLCMSynNet(skeleton=skeleton, img_size=configs.img_size, batch_size=input_batch_size, is_training=input_is_training, loss_weights=configs.loss_weights, loss_weight_fb=configs.loss_weight_fb, loss_weight_br=configs.loss_weight_br, pose_2d_scale=configs.pose_2d_scale, is_use_bn=configs.is_use_bn, nFeats=configs.nFeats, nModules=configs.nModules)
    ...
    with tf.Session() as sess:
        ...
        dlcm_syn_model.build_loss(input_maps=input_maps, input_fb=input_fb, input_br=input_br, lr=configs.learning_rate, lr_decay_step=configs.lr_decay_step, lr_decay_rate=configs.lr_decay_rate)
        ...
        while True:
            global_steps = sess.run(dlcm_syn_model.global_steps)
            ...
            for b in range(batch_size):
                ...
                if "joints_3d" in cur_label.keys():
                    ...
                    cur_img, cur_maps, cur_joints_2d, cur_bone_status, cur_bone_relations = preprocessor.preprocess_h36m(img=cur_img, joints_2d=cur_joints_2d, joints_3d=cur_joints_3d, scale=cur_scale, center=cur_center, cam_mat=cur_cam_mat, is_training=train_valid_counter.is_training)
                else:
                    # the mpii lsp datas
                    ...
                    cur_img, cur_maps, cur_joints_2d, cur_bone_status, cur_bone_relations = preprocessor.preprocess_base(img=cur_img, joints_2d=cur_joints_2d, bone_status=cur_bone_status, bone_relations=cur_bone_relations, is_training=train_valid_counter.is_training)
            ...
            if train_valid_counter.is_training:
                ...
                summary  = sess.run(
                        [
                         dlcm_syn_model.train_op,
                         dlcm_syn_model.heatmaps_acc,
                         dlcm_syn_model.fb_acc,
                         dlcm_syn_model.br_acc,
                         dlcm_syn_model.total_loss,
                         dlcm_syn_model.losses,
                         dlcm_syn_model.fb_loss,
                         dlcm_syn_model.br_loss,
                         dlcm_syn_model.lr,
                         dlcm_syn_model.merged_summary],
                        feed_dict={input_images: batch_images_np,
                            input_fb: batch_fb_np, input_br: batch_br_np,
                            ...
                            input_is_training: True, input_batch_size: configs.train_batch_size})
                ...
                summary  = sess.run(
                        ...
                        feed_dict={input_images: batch_images_np,
                            ...
                            input_heatmaps_level_1: batch_heatmaps_level_1,
                            input_heatmaps_level_2: batch_heatmaps_level_2,
                            input_is_training: False, input_batch_size: configs.valid_batch_size})
